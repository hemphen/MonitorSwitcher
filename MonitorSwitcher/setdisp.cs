...
namespace MonitorSwitcher
{
    class setdisp
    {
        ...
        static void Main(string[] args)
        {
            ...
            var pathArray = new DisplayConfigPathInfo[numPathArrayElements];
            var modeInfoArray = new DisplayConfigModeInfo[numModeInfoArrayElements];
            var additionalInfoArray = new MonitorAdditionalInfo[numModeInfoArrayElements];
            ...
            for (int p = 0; p < numPathArrayElements; p++)
            {
                ...
                if (path.sourceInfo.statusFlags.HasFlag(DisplayConfigSourceStatus.InUse) && path.targetInfo.targetAvailable)
                {
                    ...
                    Console.WriteLine($"{path.targetInfo.id,8} {hMon,12} {monInfo.monitorFriendlyDevice ?? "unknown",12} {source.ToPretty(),-30} {path.targetInfo.outputTechnology,-20}/{inp,3} {target.targetVideoSignalInfo.videoStandard} {(path.flags == 1 ? "ACTIVE" : "")}");
            ...
            for (var mi = 0; mi < numModeInfoArrayElements; mi++)
            {
                ...
                if (modeInfo.infoType == DisplayConfigModeInfoType.Target)
                {
                    try
                    {
                        var additionalInfo = GetMonitorAdditionalInfo(modeInfo.adapterId, modeInfo.id);
                        additionalInfoArray[mi] = additionalInfo;
                        Console.WriteLine($"additionalInfo[{mi}]: {additionalInfo.monitorFriendlyDevice}");
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"GetMonitorAdditionalInfo threw {ex.Message}");
                        additionalInfoArray[mi].valid = false;
                    }
                }
            }
        }
